 ...
 * - Comments (!)
 */
function tokenizeMapkeyContent(block) {
    const tokens = [];
    ...
    while ((match = actionRegex.exec(content)) !== null) {
        ...
        // Tokenize arguments
        const argTokens = tokenizeActionArguments(argsString, blockStart + verbStart + verb.length, block.id, verb);
 ...
 */
function tokenizeDocumentLevel(text) {
    const tokens = [];
    // -------------------------------------------------------------------------
    ...
    while ((match = commentRegex.exec(text)) !== null) {
        ...
        const isInsideBlock = blockRanges.some(range => commentStart >= range.start && commentEnd <= range.end);
 ...
 */
function extractMultilineContent(content, tagEnd, blockStart, blockId) {
    ...
    while (currentPos < content.length) {
        ...
        const nextLine = content.substring(nextLineStart, nextLineEnd !== -1 ? nextLineEnd : content.length);
 ...
 */
function tokenizeActionArguments(argsString, baseOffset, blockId, verb) {
    const tokens = [];
